...
import { ShieldCheck, ExternalLink, Key, Info, Zap, Music, Search, MessageSquare, Mic, Video, Image as ImageIcon, User, LogOut } from 'lucide-react';
...
const leftHub = [
  ...
  { id: 'openai', name: 'OpenAI GPT-4o', tip: '💡 Tip: 강력한 논리 추론 및 코드 생성 엔진입니다. 복잡한 구조의 글쓰기, 프로그래밍, 세밀한 데이터 해석이 필요한 전문 작업에 최적화되어 있습니다.', link: 'https://platform.openai.com/api-keys' },
  { id: 'claude', name: 'Anthropic Claude', tip: '💡 Tip: 자연스러운 문체와 장문 맥락 유지에 특화된 엔진입니다. 창의적인 소설이나 블로그 포스팅 등 감성적이고 긴 호흡의 글쓰기에 가장 적합합니다.', link: 'https://console.anthropic.com/' },
  { id: 'google_search', name: 'Google Search API', tip: '💡 Tip: 글로벌 실시간 정보 탐색 엔진입니다. 전 세계 최신 정보를 팩트체크하고 자료 조사를 수행합니다. 매일 100건의 검색을 무료로 제공합니다.', link: 'https://programmablesearchengine.google.com/' },
  { id: 'naver_search', name: 'Naver Search API', tip: '💡 Tip: 국내 트렌드 및 시장 조사 엔진입니다. 블로그, 카페, 뉴스 등 국내 데이터를 수집하며 일일 25,000건의 무료 한도로 트렌드 분석에 필수적입니다.', link: 'https://developers.naver.com/apps/#/register' },
];

const rightHub = [
  { id: 'youtube', name: 'YouTube Data API v3', tip: '💡 Tip: 멀티미디어 성과 분석 및 관리 엔진입니다. 채널 통계를 분석하고 영상 업로드를 자동화하며, 댓글 분석을 통해 유튜브 성장을 가속화합니다.', link: 'https://console.cloud.google.com/' },
  { id: 'suno', name: 'Suno V5.5 Pro Music API', tip: '💡 Tip: AI 작곡 및 배경음악 생성 엔진입니다. 텍스트 설명만으로 영상 콘텐츠에 딱 맞는 전용 BGM이나 로고송을 저작권 걱정 없이 즉시 작곡하여 제공합니다.', link: 'https://suno.com/' },
  { id: 'design', name: 'Unsplash/Pexels Assets', tip: '💡 Tip: 고화질 시각 리소스 라이브러리입니다. 수백만 장의 전문 사진과 영상을 API로 즉시 불러와 콘텐츠의 디자인 퀄리티를 전문가 수준으로 높여줍니다.', link: 'https://unsplash.com/developers' },
  { id: 'voice', name: 'ElevenLabs Voice API', tip: '💡 Tip: 감성 기반 AI 보이스 생성 엔진입니다. 사람보다 더 자연스러운 성우 음성을 생성하며, 본인의 목소리를 복제하여 퍼스널 브랜딩 콘텐츠에 활용합니다.', link: 'https://elevenlabs.io/' },
];
...
const ServiceCard = ({ service, icon }: { service: any, icon: React.ReactNode }) => (
  <div className="bg-slate-900/40 border border-slate-800/60 rounded-xl p-4 hover:border-blue-500/40 transition-all duration-300 shadow-lg mb-3">
    <div className="flex justify-between items-center mb-2">
      ...
      <span className="text-[9px] font-black text-white bg-slate-700 px-1.5 py-0.5 rounded border border-slate-600 uppercase tracking-tighter">Pending</span>
    </div>
    ...
    <div className="space-y-2 text-white">
      <div className="relative group">
        {/* 사장님 요청: 포커스 시 테두리 컬러를 파란색에서 빨간색(red-500)으로 변경 */}
        <input 
          type="password" 
      ...
      <div className="flex gap-2 text-[10px] font-black uppercase">
        <button className="flex-1 bg-emerald-700 hover:bg-emerald-600 text-white py-2 rounded-md transition-all active:scale-95 shadow-lg flex items-center justify-center gap-1">
          연결 테스트 🔄
        </button>
        <a 
          href={service.link} 
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-100 p-6 font-sans">
      <div className="max-w-[1400px] mx-auto">
        <div className="flex justify-end items-center gap-4 mb-4 text-[12px] font-bold">
          <div className="flex items-center gap-2 bg-slate-800/50 px-3 py-1.5 rounded-full border border-slate-700 text-blue-400">
            <User size={14} />
            <span>{userName} 유저님 접속중</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <section>
            ...
            {leftHub.map((s) => (<ServiceCard key={s.id} service={s} icon={s.id.includes('search') ? <Search size={18} className="text-blue-400" /> : <MessageSquare size={18} className="text-amber-400" />} />))}
          </section>
          <section>
            ...
            {rightHub.map((s) => (<ServiceCard key={s.id} service={s} icon={
